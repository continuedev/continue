...
describe('Slash Commands Integration', () => {
  const mockAssistant: AssistantConfig = {
    ...
    version: '1.0.0',
    prompts: [
      { name: 'test-prompt', prompt: 'Test prompt', description: 'Test prompt description' }
    ]
  };
  ...
  describe('System Commands Registration', () => {
    ...
    it('should categorize assistant commands correctly', () => {
      const commands = getAllSlashCommands(mockAssistant);
      const assistantCommands = commands.filter(cmd => cmd.name === 'test-prompt');
    ...
    it('should only show exit command in remote mode', () => {
      const commands = getAllSlashCommands(mockAssistant, { isRemoteMode: true });
      const commandNames = commands.map(cmd => cmd.name);
