...
describe('validateFlags', () => {
  describe('format flag validation', () => {
    ...
    test('should fail when format is used without print', () => {
      ...
      expect(result.errors[0].message).toContain('--format flag can only be used with -p/--print');
    });
    ...
    test('should fail when format value is invalid', () => {
      ...
      expect(result.errors[0].message).toContain("--format currently only supports 'json'");
    });
  });
  ...
  describe('silent flag validation', () => {
    ...
    test('should fail when silent is used without print', () => {
      ...
      expect(result.errors[0].message).toContain('--silent flag can only be used with -p/--print');
    });
  });
  ...
  describe('mode flags validation', () => {
    ...
    test('should fail when both readonly and auto are used', () => {
      ...
      expect(result.errors[0].message).toContain('Cannot use both --readonly and --auto');
    });
  });
  ...
  describe('permission flags validation', () => {
    ...
    test('should fail with empty tool names in allow', () => {
      ...
      expect(result.errors[0].message).toContain('--allow requires a tool name');
    });
    ...
    test('should fail with empty tool names in exclude', () => {
      ...
      expect(result.errors[0].message).toContain('--exclude requires a tool name');
    });
  });
