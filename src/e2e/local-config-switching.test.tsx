...
describe('Local Config Switching Investigation', () => {
  ...
  test('documents the local config switching problem', () => {
    ...
    const allInvolveLocalConfigs = brokenScenarios.every(scenario => 
      scenario.from.includes('/') || scenario.from.includes('~') || scenario.from.includes('.') ||
      scenario.to.includes('/') || scenario.to.includes('~') || scenario.to.includes('.')
    );
  ...
  test('investigates assistant slug handling for local configs', async () => {
    ...
    const { updateAssistantSlug, getAssistantSlug } = await import('../auth/workos.js');
