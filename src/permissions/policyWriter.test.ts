...
describe("policyWriter", () => {
  describe("generatePolicyRule", () => {
    ...
    it("should generate command-specific patterns for Bash tool", () => {
      ...
      expect(generatePolicyRule("Bash", { command: "git status" })).toBe("Bash(git*)");
      expect(generatePolicyRule("Bash", { command: "npm install" })).toBe("Bash(npm*)");
    });

    it("should handle complex bash commands", () => {
      expect(generatePolicyRule("Bash", { command: "ls -la && echo test" })).toBe("Bash(ls*)");
      expect(generatePolicyRule("Bash", { command: "find . -name '*.ts' | head -10" })).toBe("Bash(find*)");
      expect(generatePolicyRule("Bash", { command: "  grep -r test .  " })).toBe("Bash(grep*)");
    });
