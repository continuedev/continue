    ...
    describe(`${mode} mode`, () => {
      ...
      it(`should have correct Write tool permission in ${mode} mode`, () => {
        ...
        if (mode === 'plan') {
          ...
          console.log(`Normal mode - write_file policy: ${writeFilePolicy?.permission || 'none'}`);
        }
      });
      ...
      it(`should check tool resolution order in ${mode} mode`, () => {
        ...
        const writeRelevantPolicies = policies.filter(p => 
          p.tool === 'write_file' || p.tool === 'Write' || p.tool === '*'
        );
