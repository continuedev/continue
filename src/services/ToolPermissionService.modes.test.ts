...
describe('ToolPermissionService - Mode Functionality', () => {
  ...
  describe('mode initialization', () => {
    ...
    it('should generate correct policies for plan mode', () => {
      ...
      expect(policies.some(p => p.tool === 'Grep' && p.permission === 'allow')).toBe(true);
      expect(policies.some(p => p.tool === 'LS' && p.permission === 'allow')).toBe(true);
      
      // Should allow MCP tools
      expect(policies.some(p => p.tool === 'mcp:*' && p.permission === 'allow')).toBe(true);
      
      // Should have an exclusion policy for all other tools as fallback
      expect(policies.some(p => p.tool === '*' && p.permission === 'exclude')).toBe(true);
    });
    ...
    it('should have mode-specific policy positioning', () => {
      ...
      expect(normalFirstPolicy?.tool !== '*' || normalFirstPolicy?.permission !== 'exclude').toBe(true);
  ...
  describe('mode switching', () => {
    ...
    it('should use absolute override for plan mode (ignore non-mode policies)', () => {
      ...
      const userExcludePolicy = newPolicies.find(p => p.tool === 'excludedTool');
