...
export function mockOpenAIChat(
  ...
  const { baseURL = "https://api.openai.com", delay = 0, error = false } = options;
...
export function mockWorkOSAuth(options: {
  ...
  nock("https://api.workos.com")
    ...
    .reply(error ? 401 : 200, error ? { error: "Invalid credentials" } : {
  ...
  nock("https://api.workos.com")
    ...
    .matchHeader("authorization", `Bearer ${accessToken}`)
    .reply(error ? 401 : 200, error ? { error: "Unauthorized" } : {
      data: organizations,
    });
}
 ...
 */
export function createMockMessages(count: number = 2): ChatCompletionMessageParam[] {
