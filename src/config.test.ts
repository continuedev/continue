...
describe('config', () => {
  ...
  describe('getLlmApi()', () => {
    test('should throw error when models array is empty', () => {
      ...
      expect(() => getLlmApi(assistant, mockAuthConfig))
        .toThrow('No models found in the configured assistant');
    });
    ...
    test('should throw error when no chat models available', () => {
      ...
      expect(() => getLlmApi(assistant, mockAuthConfig))
        .toThrow('No models with the chat role found in the configured assistant');
    });
  });
});
