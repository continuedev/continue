...
describe("agent-session-ordering", () => {
  ...
  describe("getComparator", () => {
    it("should return the correct comparator for PR Status", () => {
      const comparator = getComparator("PR Status");
      const statuses = ["Closed", "Draft", "Open"];
      const sorted = statuses.sort(comparator);
      expect(sorted).toEqual(["Draft", "Open", "Closed"]);
    });

    it("should return the correct comparator for Agent Status", () => {
      const comparator = getComparator("Agent Status");
      const statuses = ["Done", "Planning", "Working"];
      const sorted = statuses.sort(comparator);
      expect(sorted).toEqual(["Planning", "Working", "Done"]);
    });

    it("should return alphabetical comparator for Creator", () => {
      const comparator = getComparator("Creator");
      const creators = ["Zebra", "Alice", "Bob"];
      const sorted = creators.sort(comparator);
      expect(sorted).toEqual(["Alice", "Bob", "Zebra"]);
    });

    it("should return alphabetical comparator for Repository", () => {
      const comparator = getComparator("Repository");
      const repos = ["zebra-repo", "alice-repo", "bob-repo"];
      const sorted = repos.sort(comparator);
      expect(sorted).toEqual(["alice-repo", "bob-repo", "zebra-repo"]);
    });
  });
});
