...
describe("E2E Scenarios", () => {
  ...
  it("duplicate detection should happen in the assistant config first and then the intected blocks", async () => {
    ...
    expect(
      errors?.[0].message.includes("Duplicate models named gpt-5 deteced"),
    );
    // Beginning of the injected blocks duplication check
    expect(errors?.[5].message.includes("Duplicate rules deteced"));
    expect(
      errors?.[5].message.includes(
        "Duplicate mcpServers named Browser search deteced",
      ),
    );
  });
