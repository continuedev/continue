...
4. Yuto Agentic calls the tool using built-in functionality or the MCP server that offers that particular tool
5. Yuto Agentic sends the result back to the model
...
Yuto Agentic includes several built-in tools which provide the model access to IDE functionality.
...
- **Create Rule Block** (`create_rule_block`): Create a new rule block in `.yutoagentic/rules`
