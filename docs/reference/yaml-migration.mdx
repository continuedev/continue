...
Create a `config.yaml` file in your Continue Global Directory (`~/.continue` on Mac, `%USERPROFILE%\.continue`) alongside your current config.yml file. If a `config.yaml` file is present, it will be loaded instead of config.yml.
...
Some deprecated config.yml settings are no longer stored in config and have been moved to be editable through the user settings (Gear Icon). If found in config.yml, they will be auto-migrated to User Settings and removed from config.yml.

The following top-level fields from config.yml have been deprecated and don't have a config.yaml equivalent:
...
The following top-level fields from config.yml have been deprecated. Most UI-related and user-specific options will move into a settings page in the UI
