...
By default, Shadow Code crawls documentation sites using a specialized crawling service that provides the best experience for most users and documentation sites.
...
<Info>
  ...
  to `~/.shadow-code/.utils`, and use it as the local crawler. **Note:** Chromium
...
- Shadow Code does not automatically re-index your docs. Use `Click to re-index` to trigger a reindex for a specific source
...
You can also use the following IDE command to force a re-index of all docs: `Shadow Code: Docs Force Re-Index`.
