...
As with [@Codebase context provider configuration](./deprecated-codebase#how-to-configure-codebase-context-provider-settings), you can adjust the reranking behavior of the `@Docs` context provider with the `nRetrieve`, `nFinal`, and `useReranking`.
