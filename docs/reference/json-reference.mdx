...
<Warning>
  **This page documents deprecated functionality.** For creating slash commands and customizing Shadow Code, please use:
...
Below are details for each property that can be set in `config.json`. The config schema code is found in `extensions/vscode/config_schema.json`.
...
```
"docs": [    {    "title": "Shadow Code",    "startUrl": "",    "faviconUrl": "",  }]
```
  ...
  "slashCommands": [
    {
      ...
      "params": { "outputDir": "~/.shadow-code/session-transcripts" }
    }
  ]
}
```
...
Write a custom slash command at your own HTTP endpoint. Set 'url' in the params object for the endpoint you have setup. The endpoint should return a sequence of string updates, which will be streamed to the Shadow Code sidebar. See our basic FastAPI example for reference.
...
Describe the issue you'd like to generate, and Shadow Code will turn into a well-formatted title and body, then give you a link to the draft so you can submit. Make sure to set the URL of the repository you want to generate issues for.
