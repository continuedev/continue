...
<Info>
  **Note:** `transformers.js` cannot be used in JetBrains IDEs. However, you can
  ...
  here](../customize/model-roles/embeddings.mdx).
</Info>
...
Models in the Claude 3, Llama 3.1/3.2, Gemini 1.5, and GPT-4o families will automatically use a [repository map](../customize/custom-providers#repository-map) during codebase retrieval, which allows the model to understand the structure of your codebase and use it to answer questions. Currently, the repository map only contains the filepaths in the codebase.