...
- Better integration with Shadow Code's Agent mode features
...
Shadow Code indexes your codebase so that it can later automatically pull in the most relevant context from throughout your workspace. This is done via a combination of embeddings-based retrieval and keyword search. By default, all embeddings are calculated locally using `transformers.js` and stored locally in `~/.shadow-code/index`.
...
Shadow Code respects `.gitignore` files in order to determine which files should not be indexed. If you'd like to exclude additional files, you can add them to a `.shadow-codeignore` file, which follows the exact same rules as `.gitignore`.

Shadow Code also supports a **global** `.shadow-codeignore` file that will be respected for all workspaces, which can be created at `~/.shadow-code/.shadow-codeignore`.

If you want to see exactly what files Shadow Code has indexed, the metadata is stored in `~/.shadow-code/index/index.sqlite`. You can use a tool like [DB Browser for SQLite](https://sqlitebrowser.org/) to view the `tag_catalog` table within this file.
