...
The `check` CLI runs your checks locally — iterate on them before rolling out to your team in CI, with fast feedback, full control over what runs, and no extra setup beyond your existing Claude credentials. You can run it yourself or add it to `AGENTS.md` so your coding agent runs checks automatically.
