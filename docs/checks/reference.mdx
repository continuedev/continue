...
| `tools` | No | list | MCP servers to make available. Hub slugs or inline config. |
...
Write concrete pass/fail criteria. See [Writing Effective Checks](/checks/writing-good-checks) for guidance on prompt structure.
...
Checks do not have an `on:` field — they always run when a PR is opened. For event-triggered automation, use [agents](/agents/overview).
