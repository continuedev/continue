...
Check files live in `.yutoagentic/checks/` or `.agents/checks/` at the repository root.

## Frontmatter fields

| Field | Required | Type | Description |
|-------|----------|------|-------------|
| `name` | Yes | string | Display name shown in GitHub status checks and on yutoagentic.dev |
...
```markdown .yutoagentic/checks/migration-safety.md
---
name: Migration Safety
description: Flag destructive database migrations
...
Yuto Agentic locates checks by scanning specific directories at the repository root:

- Reads all `.md` files in `.yutoagentic/checks/`
...
`.yutoagentic/checks/` and `.agents/checks/` are equivalent. Both are scanned and merged into a single list of checks.

`.agents/` is an emerging cross-tool standard for AI agent configuration. `.yutoagentic/` is Yuto Agentic-specific. Use whichever fits your project. If both directories contain a check with the same `name`, the `.yutoagentic/checks/` version takes precedence.
