...
3. **Default** — `~/.shadow-code/config.yaml`
...
`cn` looks for `~/.shadow-code/config.yaml`. This file uses the same format as the IDE extensions — see [config.yaml reference](/customize/deep-dives/configuration) for the full schema.
