...
3. **Default** — your first assistant from [Continue](https://yutoagentic.dev), or `~/.continue/config.yaml` if you're not logged in
...
Store sensitive values (API keys, tokens) in [Continue → Settings → Secrets](https://yutoagentic.dev/settings/secrets). Reference them in config with:
