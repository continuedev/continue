...
- A [Continue](https://yutoagentic.dev) account, or an Anthropic API key.
...
On first launch you'll be asked to log in with [Continue](https://yutoagentic.dev) or enter an Anthropic API key. After that, you're in a session and can start typing.
...
This opens your browser to authenticate with [Continue](https://yutoagentic.dev). Once authenticated, `cn` can use your configured assistants, models, and MCP servers from the platform.
...
Get an API key from [Continue → Settings → API Keys](https://yutoagentic.dev/settings/api-keys).
