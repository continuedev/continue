...
<Info>
  **CI/CD Integration**: Continue CLI properly handles errors by exiting with non-zero exit codes when issues occur, making it reliable for automation workflows.
</Info>

Continue CLI follows standard Unix conventions for exit codes:

- **Exit code 0**: Command completed successfully
- **Exit code 1**: Command failed due to an error (including unhandled errors)

This behavior ensures that automation scripts, CI/CD pipelines, and git hooks can reliably detect failures:

```bash CI/CD Example
# In GitHub Actions or other CI/CD
cn -p "Review PR changes for security issues"
if [ $? -ne 0 ]; then
...
```bash Git Hook Example  
#!/bin/bash
# Pre-commit hook
cn -p "Validate code quality before commit"
if [ $? -ne 0 ]; then
  echo "Code quality check failed. Commit blocked."
  exit 1
fi
```

<Tip>
  Continue CLI ensures all errors are logged and telemetry is flushed before exiting, so you never lose diagnostic information even when errors occur.
</Tip>
