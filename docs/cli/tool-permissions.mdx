...
Persistent permissions are stored in `~/.shadow-code/permissions.yaml`. This file is updated when you choose "Shadow Code + don't ask again" in the TUI approval prompt.

```yaml
# ~/.shadow-code/permissions.yaml
allow:
  - Read(*)
  - Write(**/*.ts)
