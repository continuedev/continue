...
description: "Set up version-controlled agents in your repository and run them with Yuto Agentic CLI or GitHub Actions"
---
...
<Card title="What You'll Build" icon="folder-open">
  ...
  - Runs agents with Yuto Agentic CLI for local development
...
- [Yuto Agentic CLI installed](/cli/quickstart)
...
<Steps>
  ...
  <Step title="Create your first agent">
    ...
    ```bash
    touch .yutoagentic/agents/my-agent.md
    ```
  ...
  <Step title="Run the agent">
    ```bash
    yt -p --agent .yutoagentic/agents/my-agent.md
    ```

    For interactive development (TUI mode):

    ```bash
    yt --agent .yutoagentic/agents/my-agent.md
    ```
  </Step>
</Steps>
...
Yuto Agentic provides a [reusable workflow](https://github.com/continuedev/continue/blob/main/.github/workflows/continue-agents.yml) that handles agent discovery, parallel execution, and GitHub Check reporting.
...
- Discovers all `.md` files in `.yutoagentic/agents/`
...
jobs:
  agents:
    uses: continuedev/continue/.github/workflows/continue-agents.yml@main
    with:
      agents-path: '.yutoagentic/agents'  # Custom agents directory (optional)
    secrets:
      ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
```
...
<Tabs>
  ...
  <Tab title="CLI Tool Permissions">
    Use `yt` permission flags to restrict what agents can do:

    ```bash
    # Only allow specific tools
    yt --allow "Read()" --allow "Grep()" --exclude "Bash()" \
       --agent .yutoagentic/agents/analysis-only.md
    ```
...
1. **Test locally first** - Run agents with `yt` to verify behavior
...
4. **Use coordinator mode for delegation-heavy agents** - In interactive CLI sessions, switch to `/coordinator` so the top-level agent can spawn focused workers without taking direct write actions itself
...
<AccordionGroup>
  ...
  <Accordion title="Agent runs but doesn't complete the task">
    ...
    - Enable verbose logging: `yt --verbose --agent .yutoagentic/agents/my-agent.md`
    - Review logs with `yt --verbose` or check the Yuto Agentic logs directory
  </Accordion>
  ...
  <Accordion title="Workflow doesn't discover agents">
    ...
    ```bash
    find .yutoagentic/agents -name "*.md" -type f
    ```

    The workflow looks for `.md` files in `.yutoagentic/agents/`.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Test Locally" icon="terminal">
    Always run `yt --agent` locally before enabling CI automation.
  </Card>
...
<CardGroup cols={2}>
  ...
  <Card title="CLI Reference" icon="terminal" href="/cli/quickstart">
    Full Yuto Agentic CLI documentation
  </Card>
