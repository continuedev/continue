...
<Tip>
  **Test in Plan Mode First**: Before making performance optimizations that might affect your site,
  test your prompts in plan mode (see the [Plan Mode Guide](/guides/plan-mode-guide); press **Shift+Tab** to switch modes). This shows you
...
<Info>
  **Enhanced Analytics**: Combine Netlify's A/B testing with PostHog session recordings
  ...
  See our [PostHog session analysis guide](/guides/posthog-github-continuous-ai) to set up
...
<Tip>
  ...
  production deployment, blocking any changes that violate your performance budgets.
</Tip>
...
<Warning>
  **Note**: Netlify Analytics Pro requires a paid Netlify Pro account ($19/month per site).
  The prompt above assumes you have this plan. For free tier users, focus on steps 2-5 which
  use Netlify Functions and Edge Functions to build custom analytics.
</Warning>
