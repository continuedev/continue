...
If you have GitHub MCP installed, authentication is simplified. Otherwise, add your credentials:

```bash
# In cn TUI mode:
"Set up PostHog and GitHub credentials:
- POSTHOG_API_KEY: My PostHog Personal API Key
- POSTHOG_PROJECT_ID: Extract from PostHog URL
- POSTHOG_HOST: https://us.posthog.com or custom host
- GitHub: Use MCP tool or PAT if not configured
Test both API connections to verify setup."
```

<Tip>
  With GitHub MCP configured, you don't need a separate GitHub PAT - the MCP
  tool handles authentication automatically.
</Tip>
...
With GitHub MCP, issue creation is simplified:

```bash
# In cn TUI mode:
"Create GitHub issues from the PostHog analysis:
- Use GitHub MCP tool to create issues
- Add labels: bug, user-experience, automated
- Set priority labels (high/medium/low)
- Title format: '🔍 UX Issue: [title]'
- Include session data and technical details
Confirm each issue was created with URL."
```

<Info>
  **Using GitHub MCP**: If configured, the MCP tool handles authentication and
  API calls automatically. Otherwise, Continue will use the GitHub API directly
  with your PAT.
</Info>
    ...
    steps:
      ...
      - name: Run PostHog Session Analysis
        run: |
          ...
          4. Create GitHub issues with labels and priority
          5. Use GitHub MCP if available, otherwise API
...
2. Verify GitHub MCP tool is working
...
| GitHub issues not created | Verify GitHub MCP is configured or PAT has issues:write |
...
✅ **Level 2 Continuous AI** - AI analyzes data and creates tasks with human review  
✅ **MCP Integration** - Natural language commands replace manual API calls  
✅ **Scalable Workflow** - Handles growing session data automatically
...
- Install [GitHub MCP](https://hub.continue.dev/github/github-mcp) for better integration
...
- [GitHub MCP on Continue Hub](https://hub.continue.dev/github/github-mcp)
