...
To integrate your custom RAG system with Yuto Agentic, you'll create an MCP (Model Context Protocol) server. MCP provides a standardized way for AI tools to access external resources.
...
async def search_codebase(query: str, limit: int = 10) -> list[TextContent]:
    ...
    # Format results for Yuto Agentic
    formatted_results = []
...
### Configure Yuto Agentic to use your MCP server

Add your MCP server to Yuto Agentic's configuration:
