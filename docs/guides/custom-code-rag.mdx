...
To integrate your custom RAG system with Shadow Code, you'll create an MCP (Model Context Protocol) server. MCP provides a standardized way for AI tools to access external resources.
...
async def search_codebase(query: str, limit: int = 10) -> list[TextContent]:
    ...
    # Format results for Shadow Code
    formatted_results = []
...
### Configure Shadow Code to use your MCP server

Add your MCP server to Shadow Code's configuration:
