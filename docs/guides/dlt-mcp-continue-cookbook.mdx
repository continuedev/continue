---
title: "Building Data Pipelines with dlt Hub MCP and Continue"
...
sidebarTitle: "dlt Hub Data Pipelines with Continue"
---

<Card title="What You'll Build" icon="database">
  An AI-powered data pipeline development system that uses Continue's AI agent with dlt Hub
  ...
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Load the Pre-Built Agent">
        ...
        This agent includes:
        - **dlt Hub MCP** pre-configured and ready to use
    ...
    <Info>
      **Why Use the Agent?** The pre-built agent provides consistent pipeline development workflows and handles MCP configuration automatically, making it easier to get started with AI-powered data engineering.
    </Info>
...
<Steps>
  ...
  <Step title="Connect dlt Hub MCP via Continue Hub">
    Visit the [dlt Hub MCP on Continue Hub](https://hub.continue.dev/dlthub/dlt-mcp) and click **Install** to add it to the agent you created in the step above.

    This will add dlt Hub MCP to your agent's available tools. The Hub listing automatically configures the MCP command.
    ...
    <Tip>
      **Alternative installation methods:**
      ...
      Once installed, dlt Hub MCP tools become available to your Continue agent for all prompts.
    </Tip>
...
<Accordion title="Agent Requirements">
    ...
    The agent will automatically detect and use your configuration along with the pre-configured dlt Hub MCP for pipeline operations.

</Accordion>

---

## dlt Hub MCP vs dlt+ MCP

<Card title="Understanding the Difference" icon="info-circle">
  **dlt Hub MCP** is focused on local pipeline development and inspection. It provides tools to:
  ...
  **dlt+ MCP** extends these capabilities with cloud-based features for production deployments:
  ...
  For local development and getting started, **dlt Hub MCP** is the right choice. Consider **dlt+ MCP** when you need production deployment features and team collaboration.
</Card>
...
Now you can use natural language prompts to develop and debug your dlt pipelines. The Continue agent automatically calls the appropriate dlt Hub MCP tools.
...
This example demonstrates a **Continuous AI workflow** where data pipeline validation runs automatically in your CI/CD pipeline in headless mode (`cn -p`) with dlt Hub MCP.
...
```yaml
name: Data Pipeline Validation with dlt Hub MCP
...
<Info>
  The dlt Hub MCP works with your local pipeline state. Make sure your CI environment
...
<Check>
  **Verification Steps:**
  - dlt Hub MCP is installed via [Continue Hub](https://hub.continue.dev/dlthub/dlt-mcp)
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    href="https://dlthub.com/blog/deep-dive-assistants-mcp-continue"
  >
    Deep dive into dlt Hub MCP integration
  </Card>
</CardGroup>