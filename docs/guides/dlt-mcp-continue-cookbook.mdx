...
<Card title="🚀 Fastest Path to Success" icon="zap">
  ...
  the dlt MCP and optimized data pipeline workflows for more consistent results.
</Card>
...
<Tabs>
  <Tab title="⚡ Quick Start (Recommended)">
    ...
    <Info>
      **Why Use the Agent?** The pre-built [dlt Assistant agent](https://hub.continue.dev/dlthub/dlt-assistant) provides consistent pipeline development workflows and handles MCP configuration automatically, making it easier to get started with AI-powered data engineering.
    </Info>
...
This example demonstrates a **Continuous AI workflow** where data pipeline validation runs automatically in your CI/CD pipeline in headless mode using the [dlt Assistant agent](https://hub.continue.dev/dlthub/dlt-assistant).
...
<Info>
  The workflow uses the pre-built [dlt Assistant agent](https://hub.continue.dev/dlthub/dlt-assistant) with `--config dlthub/dlt-assistant`. This agent comes pre-configured with the dlt MCP and optimized rules for pipeline operations.
</Info>
  ...
  validate-pipeline:
    ...
    steps:
      ...
      - name: Generate Pipeline Report
        ...
        run: |
          cn --config dlthub/dlt-assistant \
             -p "Generate a markdown report summarizing:
