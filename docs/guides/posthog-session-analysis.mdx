---
title: "PostHog Session Recording Analysis with Continue CLI"
description: "Automatically analyze PostHog session recordings to identify UX issues and generate code optimization suggestions using Continue CLI"
---
...
<Warning>
  **Session Recordings Required**: This workflow requires PostHog session
  recordings to be enabled. If you haven't set this up yet, follow [PostHog's
  session recording installation
  guide](https://posthog.com/docs/session-replay/installation) first.
</Warning>
...
<Steps>
  ...
  <Step title="Install & Setup GitHub CLI">
    ```bash # macOS brew install gh # Ubuntu/Debian sudo apt install gh #
...
<Info>
  **What This Script Does:** 1. **Fetches** recent session recordings from
  PostHog API 2. **Filters** for problematic sessions (errors or long durations)
  3. **Analyzes** with Continue CLI to identify specific UX issues 4.
...
<Tip>
  **Smart Label Assignment**: The script automatically assigns labels based on
  issue priority: - **High Priority** → `bug`, `high-priority`,
  `user-experience` - **Medium Priority** → `enhancement`, `medium-priority`,
  `user-experience` - **Low Priority** → `low-priority`, `user-experience`
</Tip>
...
Create a file at `.github/workflows/daily-analysis.yml`:
...
**Required GitHub Secrets:** - `POSTHOG_API_KEY`: Your PostHog Personal API Key - `POSTHOG_PROJECT_ID`: Your PostHog Project ID - `GH_PAT`: Your GitHub Personal Access Token
...
<Check>
  **Success Indicators:** - PostHog API returns session data (not empty) -
  ...
  created with proper labels and formatting - No error messages in the console
  output
</Check>
...
<Card title="This is True Continuous AI" icon="rocket">
...
<Warning>
  **Protect Your API Keys:** - Never commit `.env` files or secrets to version
  control - Use GitHub Secrets for automation workflows - Limit token scopes to
...
Ready to enhance your workflow? Consider these additions:

<CardGroup cols={2}>
  <Card
    title="MCP Integration"
    icon="plug"
    href="/guides/posthog-session-analysis#enhanced-workflow-with-mcp-tools"
  >
    Add PostHog and GitHub MCP servers for more sophisticated analysis
  </Card>
...
<Card title="Want More Advanced Features?" icon="star">
  Check out the [Enhanced MCP
  Integration](/guides/posthog-session-analysis#enhanced-workflow-with-mcp-tools)
  section for direct PostHog and GitHub API access through structured tools.
</Card>
