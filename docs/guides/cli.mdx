...
To switch between configurations, you can use the `/config` slash command in `cn`, or you can start it with the `--config` flag (e.g. `cn --config continuedev/default-config` or `cn --config ~/.continue/config.yaml`).
