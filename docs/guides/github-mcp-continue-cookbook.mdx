---
title: "GitHub Issues and PRs with GitHub MCP and Yuto Agentic"
description: "Use Yuto Agentic and the GitHub MCP to list, summarize, and act on open issues and recently merged pull requests with natural language prompts."
sidebarTitle: "GitHub Issues with Yuto Agentic"
---
...
<Card title="What You'll Build" icon="github">
  A GitHub workflow assistant that uses Yuto Agentic with the GitHub MCP to:
...
<Step title="Configure GitHub Token">
  Add your `GITHUB_TOKEN` to your [Yuto Agentic Mission Control agent's environment variables](https://yutoagentic.dev/settings).
  </Step>
</Steps>
...
<Card title="🚀 Fastest Path to Success" icon="rocket">
  Use the GitHub MCP from Yuto Agentic Mission Control for one-click setup, or add it via CLI.
</Card>
...
<Tabs>
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Install GitHub MCP via Yuto Agentic Mission Control">
        Visit the [GitHub Project Manager Agent](https://yutoagentic.dev/continuedev/github-project-manager-agent) on Yuto Agentic Mission Control and click **Install** to add it to your agent.
      ...
      <Step title="Launch the Agent">
        From your repo root:
        ```bash
        yt --agent continuedev/github-project-manager-agent
        ```
    ...
    <Info>
      You can also attach an MCP to a one-off session: `yt --mcp anthropic/github-mcp`.
    </Info>
  </Tab>
...
<Accordion title="Agent Requirements">
    To use GitHub MCP with Yuto Agentic CLI, you need either:
...
<Info>
  **Where to run these workflows:**
  ...
  - **CLI (headless mode)**: Use `yt -p "your prompt" --auto` for automation
...
- `CONTINUE_API_KEY`: From [yutoagentic.dev/settings/api-keys](https://yutoagentic.dev/settings/api-keys)
  ...
  report:
    ...
    steps:
      ...
      - name: Post PR Context Comment
        ...
        run: |
          REPORT=$(        yt --agent continuedev/github-project-manager-agent \
...
<CardGroup cols={2}>
  <Card title="GitHub MCP (Anthropic)" icon="link" href="https://yutoagentic.dev/anthropic/github-mcp">
    Anthropic GitHub MCP on Yuto Agentic Mission Control
  </Card>
  <Card title="MCP in Yuto Agentic" icon="book" href="/customize/deep-dives/mcp">
    How MCP works with Yuto Agentic agents
  </Card>
  <Card title="GitHub Project Manager Agent" icon="robot" href="https://yutoagentic.dev/continuedev/github-project-manager-agent">
    Pre-configured agent on Yuto Agentic Mission Control
  </Card>
