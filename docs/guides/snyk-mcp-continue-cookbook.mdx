---
title: "Automated Security Scanning with Snyk MCP and Yuto Agentic"
...
sidebarTitle: "Snyk Security Scanning with Yuto Agentic"
---
...
<Card title="What You'll Build" icon="shield-halved">
  An automated security scanning system that uses Yuto Agentic's AI agent with Snyk
...
<iframe
  ...
  title="Snyk MCP Yuto Agentic Cookbook Demo"
  frameborder="0"
...
<Note>
  **Important**: The Snyk MCP requires the Snyk CLI to be authenticated locally. Run `snyk auth` to authenticate before using the Yuto Agentic agent with Snyk MCP.
</Note>
 <Warning>
  ...
  <Tab title="⚡ Quick Start (Recommended)">
    <Steps>
      <Step title="Load the Pre-Built Agent">
        ...
        ```bash
        yt --agent continuedev/snyk-continuous-ai-agent
        ```
      ...
      <Step title="Run Your First Security Scan">
        ...
        ```bash
         # Headless mode
        yt -p "Run a complete security scan on this project including code vulnerabilities, dependencies, and any IaC files. Summarize findings by severity." --auto
        ```
...
<Steps>
  <Step title="Create a New Agent via the Yuto Agentic Mission Control">
    Go to the [Yuto Agentic Mission Control](https://yutoagentic.dev) and [create a new agent](https://yutoagentic.dev/agents/new).
    </Step>

  <Step title="Connect Snyk MCP via Yuto Agentic Mission Control">
    Visit the [Snyk Continuous AI Agent](https://yutoagentic.dev/continuedev/snyk-continuous-ai-agent) and click **Install** to add it to the agent you created in the step above.
    ...
    <Tip>
      **Alternative installation methods:**
      1. **Quick CLI install**: `yt --mcp snyk/snyk-mcp`
      2. **Manual configuration**: Add the MCP to your `~/.yutoagentic/config.json` under the `mcpServers` section

      Once installed, Snyk MCP tools become available to your Yuto Agentic agent for all prompts.
    </Tip>

    <Info>
      The MCP will request authentication and folder trust permissions when first used.
      This is handled automatically by the Yuto Agentic agent.
    </Info>

  </Step>

  <Step title="Add Secure-at-Inception Rules">
    Install the [Snyk Secure-at-Inception rules](https://yutoagentic.dev/snyk/secure-at-inception) from Mission Control to enable automatic security scanning.
    ...
    **How to add rules to your agent:**
    ...
    3. Rules apply globally to all your Yuto Agentic sessions
  ...
  <Step title="Run Your First Security Scan">
    ...
    ```bash
    # TUI mode
    yt "Run a complete security scan on this project including code vulnerabilities, dependencies, and any IaC files. Summarize findings by severity."
    ```
  </Step>
</Steps>
...
<Accordion title="Agent Requirements">
    To use the pre-built agent, you need either: 
      ...
      - **Your own API keys** added to Yuto Agentic Mission Control secrets (same as manual setup)
...
Now you can use natural language prompts to run comprehensive security scans. The Yuto Agentic agent automatically calls the appropriate Snyk MCP tools.

<Info>
You can add prompts to your agent's configuration for easy access in future sessions. Go to your agent in the [Yuto Agentic Mission Control](https://yutoagentic.dev), click **Edit**, and add prompts under the **Prompts** section.
</Info>

<Info>
  **Where to run these workflows:**
  - **IDE Extensions**: Use Yuto Agentic in VS Code, JetBrains, or other supported IDEs
  - **Terminal (TUI mode)**: Run `yt` to enter interactive mode, then type your prompts
  - **CLI (headless mode)**: Use `yt -p "your prompt" --auto` for headless commands
...
```bash
yt -p "Run a Snyk Code scan on this repo with severity threshold medium. Summarize issues with file:line. Propose minimal diffs for the top 3 and rerun to verify." --auto
```
...
**Headless Mode Prompt:**
```bash
yt -p "Run Snyk Open Source on this repo (include dev deps). Summarize vulnerable paths and propose a minimal-risk upgrade plan. Re-test after the plan (dry run)." --auto
```
...
**Headless Mode Prompt:**
```bash
yt -p "Scan ./infra with Snyk IaC. Report high/critical misconfigs with exact files/lines. Provide code changes and re-scan to confirm." --auto
```
...
**Headless Mode Prompt:**
```bash
yt -p "Scan image my-api:latest. Exclude base image vulns. Print dependency tree. Recommend a safer base image or upgrades. Re-test after the change (dry run)." --auto
```
...
**Headless Mode Prompt:**
```bash
yt -p "Scan only files changed since origin/main with Snyk Code. Block if new high issues would be introduced. Show deltas." --auto
```
...
**Headless Mode Prompt:**
```bash
yt -p "Open Snyk Learn lessons related to the top CWE(s) from this scan." --auto
```
...
- `CONTINUE_API_KEY`: Your Yuto Agentic API key from [yutoagentic.dev/settings/api-keys](https://yutoagentic.dev/settings/api-keys)
...
jobs:
  security-scan:
    ...
    steps:
      ...
      - name: Install Yuto Agentic CLI
        run: |
          npm install -g @yutoagentic/cli
          echo "✅ Yuto Agentic CLI installed"
      ...
      - name: Generate AI Mitigation Suggestions
        ...
        run: |
          ...
          # Create a summary of findings for Yuto Agentic CLI
          ...
          if [ -n "$FINDINGS_SUMMARY" ]; then
            ...
            # Use Yuto Agentic CLI to generate mitigation suggestions
            ...
            yt --agent continuedev/snyk-continuous-ai-agent -p "$PROMPT" --auto > mitigation-suggestions.md || {
      ...
      - name: Post Mitigation Summary to PR
        ...
        run: |
          ...
          **Scan Details:**
          ...
          - 🤖 Generated with Yuto Agentic CLI + Snyk
...
<Info>
  **About SNYK_TOKEN**: The workflow uses the SNYK_TOKEN in two ways:
  ...
  2. **Yuto Agentic CLI access** - Available as an environment variable when Yuto Agentic generates AI mitigation suggestions

  The `yt` agent automatically uses the SNYK_TOKEN when needed for Snyk MCP operations.
</Info>

<Info>
  This workflow demonstrates several advanced features:
  - **Changed Files Detection**: Only scans files modified in the PR
  - **AI Mitigation**: Uses Yuto Agentic CLI to generate actionable mitigation steps
...
Implement automated security policies using Yuto Agentic's rule system. See the [Rules deep dive](/customize/deep-dives/rules) for authoring tips.

<Note>
  **Coming Soon**: These security guardrail prompts will be available as pre-configured rules on the Yuto Agentic Mission Control for easy installation.
</Note>
...
<Check>
  **Verification Steps:** - Snyk MCP is installed via [Yuto Agentic
  Hub](https://yutoagentic.dev/snyk/snyk-mcp) - Secure-at-Inception rules are
  [enabled](https://yutoagentic.dev/snyk/secure-at-inception) - Authentication
...
<CardGroup cols={2}>
  ...
  <Card title="Yuto Agentic Mission Control" icon="plug" href="https://yutoagentic.dev">
