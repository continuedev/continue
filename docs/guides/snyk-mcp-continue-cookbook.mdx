---
title: "Automated Security Scanning with Snyk MCP and Shadow Code"
...
sidebarTitle: "Snyk Security Scanning with Shadow Code"
---
...
<Card title="What You'll Build" icon="shield-halved">
  An automated security scanning system that uses Shadow Code's AI agent with Snyk
...
<iframe
  ...
  title="Snyk MCP Shadow Code Cookbook Demo"
  frameborder="0"
...
<Note>
  **Important**: The Snyk MCP requires the Snyk CLI to be authenticated locally. Run `snyk auth` to authenticate before using the Shadow Code agent with Snyk MCP.
</Note>
 <Warning>
...
<Steps>
  <Step title="Add Snyk MCP to Shadow Code CLI">
    ...
    <Tip>
      **Installation methods:**
      ...
      Once installed, Snyk MCP tools become available to your Shadow Code agent for all prompts.
    </Tip>

    <Info>
      The MCP will request authentication and folder trust permissions when first used.
      This is handled automatically by the Shadow Code agent.
    </Info>
...
Now you can use natural language prompts to run comprehensive security scans. The Shadow Code agent automatically calls the appropriate Snyk MCP tools.
    ...
    steps:
      ...
      - name: Generate AI Mitigation Suggestions
        ...
        run: |
          ...
          # Create a summary of findings for Shadow Code CLI
          ...
          if [ -n "$FINDINGS_SUMMARY" ]; then
            ...
            # Use Shadow Code CLI to generate mitigation suggestions
      ...
      - name: Post Mitigation Summary to PR
        ...
        run: |
          ...
          **Scan Details:**
          ...
          - 🤖 Generated with Shadow Code CLI + Snyk
...
<Info>
  **About SNYK_TOKEN**: The workflow uses the SNYK_TOKEN in two ways:
  ...
  2. **Shadow Code CLI access** - Available as an environment variable when Shadow Code generates AI mitigation suggestions
...
<Info>
  ...
  - **AI Mitigation**: Uses Shadow Code CLI to generate actionable mitigation steps
...
Implement automated security policies using Shadow Code's rule system. See the [Rules deep dive](/customize/deep-dives/rules) for authoring tips.
