---
title: "Using Plan Mode with Yuto Agentic"
...
If you are working in the CLI, `yt` also exposes adjacent execution profiles through `/mode`: `explore` for read-heavy reconnaissance, `verify` for review-first validation, and `coordinator` for delegating work to subagents while keeping direct writes blocked on the coordinator.
...
Context is the foundation of effective planning. Without proper context, AI models fall back on generic patterns, leading to plans that don't fit your specific system. Yuto Agentic's [context system](/ide-extensions/chat/context-selection) transforms broad suggestions into actionable strategies:
