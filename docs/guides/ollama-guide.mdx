---
title: "Using Ollama with Shadow Code: A Developer's Guide"
description: "Complete guide to setting up Ollama with Shadow Code for local AI development. Learn installation, configuration, model selection, performance optimization, and troubleshooting for privacy-focused offline coding assistance"
---
...
- Shadow Code extension installed
...
## How to Configure Ollama with Shadow Code

There are multiple ways to configure Ollama models in Shadow Code:
...
```yaml title="~/.shadow-code/configs/config.yaml"
...
<Warning>
  **Important**: Blocks only provide configuration - you still need to pull
  the model locally. The block `ollama/deepseek-r1-32b` configures Shadow Code
...
Shadow Code can automatically detect available Ollama models. You can configure this in your YAML:

```yaml title="~/.shadow-code/config.yaml"
models:
  - name: Autodetect
    provider: ollama
    model: AUTODETECT
...
3. Shadow Code will scan for available Ollama models
...
<Note>
  ...
  available models. When set to `AUTODETECT`, Shadow Code will dynamically populate
...
Shadow Code may use a higher default context length than other tools. Reduce `contextLength` in your config (e.g., to 2048), or try a smaller model. See [Ollama provider troubleshooting](/customize/model-providers/top-level/ollama#troubleshooting) for details.
...
```yaml
# ~/.shadow-code/configs/config.yaml
name: Local Config
version: 0.0.1
schema: v1
models:
...
async def create_user(user: User):
    # Shadow Code will help complete this implementation
...
Use Shadow Code with Ollama to:
...
_This guide is based on Ollama v0.11.x and Shadow Code v1.1.x. Please check for updates regularly._
