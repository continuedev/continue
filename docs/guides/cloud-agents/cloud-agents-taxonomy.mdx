...
Cloud agents exist to **handle work that benefits from reasoning, shared context, and reviewability** — whether it runs once or repeatedly.
...
<CardGroup cols={2}>
  ...
  <Card title="Not Just Cron Jobs">
    Cron is a trigger.  
    Cloud agents reason over changing inputs and produce reviewable outcomes.
  </Card>

  <Card title="Not One-Off Scripts">
    Scripts execute fixed logic.  
    Cloud agents apply reasoning and shared context — even for a single run.
  </Card>
...
One-off cloud agents are first-class agents.  
They differ by trigger, not by capability.
...
> **Agents alone don’t scale without control and visibility do.**
