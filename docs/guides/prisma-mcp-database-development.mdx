...
## Step 1: Set Up Prisma MCP

Configure Prisma MCP to enable AI-powered database development.

<Tabs>
  <Tab title="Configure Prisma MCP">
    ...
    **Quick Setup**
    ...
    The Prisma MCP local server includes everything you need:
  ...
  <Tab title="Configure for Continue CLI">

    **Add to Continue CLI Configuration**
    ...
    **For Continue CLI via command:**
    ```bash
    claude mcp add prisma-local npx prisma mcp
    ```

    <Tip>
      The Prisma MCP server starts automatically when Continue CLI needs it. No manual server management required!
    </Tip>
...
<Tabs>
  ...
  <Tab title="🛠️ Manual Setup">
    <Steps>
      <Step title="Add Prisma MCP to Continue CLI">
        Configure the Prisma MCP:
