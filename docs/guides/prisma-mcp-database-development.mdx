...
- Use [Prisma MCP](https://www.prisma.io/docs/postgres/integrations/mcp-server) to build database workflows
...
<Tip>
  Continue CLI with Prisma MCP can generate complete database schemas, write migrations, and even create realistic seed data - you just describe what you need!
</Tip>

## Step 1: Understanding Prisma MCP

<Info>
  **Prisma MCP Variants**:
  - **Local Server**: For individual developers working on their own machines
  - **Remote Server**: For platform builders offering database management to users

  This guide uses the **Local Server** variant.
</Info>

**Available Prisma MCP Tools**:
...
## Step 2: Prisma Postgres Setup (Optional)
...
<Tabs>
  ...
  <Tab title="Local PostgreSQL Setup">
...
## Step 3: Prisma Database Workflow Options
...
<Info>
  **How Prisma MCP Works**:
  - Runs locally as a Node.js process via `npx`
...
<Tabs>
    ...
    **Perfect for:** Rapid database development with AI-powered schema design and migration generation
    ...
    <Steps>
      <Step title="Add the Pre-Built Agent">
        ...
        - **[Prisma MCP](https://www.prisma.io/docs/postgres/integrations/mcp-server)** for reliable database operations
  ...
  <Tab title="🛠️ Manual Setup">
    <Steps>
      <Step title="Create a Custom Agent on Continue Hub">
        1. Go to [Continue Hub](https://hub.continue.dev) and create a new agent
        2. Add Prisma MCP to your agent's configuration:
      ...
      <Step title="Create Custom Database Development Prompts">
        Use this prompt template with Continue CLI:
...
<Accordion title="Agent Requirements">
  ...
  The agent will automatically detect and use your configuration. For Prisma MCP:
  - **Node.js 18+** installed locally
  - **PostgreSQL database** (local or hosted)
  - Prisma MCP runs automatically via `npx`
</Accordion>

---

## Step 4: Design Your Database Schema with AI
...
<Info>
  **Prisma MCP Capabilities for Schema Design**:
...
## Step 6: Seed Your Database with Realistic Data
...
## Step 7: Launch Prisma Studio for Visual Management
...
## Step 8: Automate Database Health Checks
...
### Prisma MCP Connection Issues

If you encounter issues with Prisma MCP:

1. Verify Node.js version is 18 or higher: `node --version`
2. Ensure Prisma is accessible: `npx prisma --version`
3. Check your DATABASE_URL is correctly set in `.env`
4. Try running Prisma commands directly: `npx prisma migrate status`
...
When Prisma MCP detects destructive operations (like `migrate reset`):
...
<Info>
  This safety feature prevents accidental data loss when AI agents run database commands.
</Info>
