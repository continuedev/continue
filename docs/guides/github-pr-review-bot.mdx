...
<Card title="What You'll Build" icon="code-pull-request">
  ...
  - Runs entirely in GitHub Actions (your code never leaves your repo)
...
**Privacy-First** - Unlike SaaS review tools, your code analysis happens in your GitHub Actions runner. Your code never leaves your infrastructure.

**Customizable** - Define team-specific rules in `.continue/rules/` that automatically apply to every pull request.

**Powered by Continue** - Leverage Continue's AI agent for intelligent, context-aware reviews with full control over your configuration.
...
**Required:**
- `CONTINUE_API_KEY` - Your Continue API key from [hub.continue.dev](https://hub.continue.dev/settings/api-keys)

**Optional (for better permissions):**
- `APP_ID` - GitHub App ID (for enhanced API rate limits)
...
<Accordion title="Setting up a GitHub App (Optional but Recommended)">
  ...
  5. Add `APP_ID` and `APP_PRIVATE_KEY` to repository secrets
...
Replace `continuedev/code-reviewer` with your own config:
...
- ✅ **Privacy-first** - Code analysis happens in your GitHub Actions, not external servers
