---
title: "Code Review Bot with Yuto Agentic and GitHub Actions"
description: "Set up automated, context-aware pull request reviews using Yuto Agentic CLI in GitHub Actions - privacy-first with custom rules"
...
<CardGroup cols={3}>
  <Card title="Privacy-First" icon="shield">
    All logs and processing happen in your runner: Yuto Agentic CLI runs in GitHub Actions → code to your LLM provider (OpenAI, Anthropic, etc.). No hosted Yuto Agentic service reads your code.

  </Card>

  <Card title="Customizable" icon="sliders">
    Define team-specific rules in `.yutoagentic/rules/` that automatically apply to every pull request.

  </Card>

  <Card title="Context Awareness" icon="robot">
    Leverage Yuto Agentic's AI agent for intelligent, context-aware reviews with full control over your configuration.
...
- A Yuto Agentic API key from [yutoagentic.dev/settings/api-keys](https://yutoagentic.dev/settings/api-keys)
- Yuto Agentic assistant configured for code reviews (or use our recommended default)

<Info>
  **Want to customize the review bot?**
  
  You can remix the default review bot configuration at [yutoagentic.dev/continuedev/review-bot](https://yutoagentic.dev/continuedev/review-bot) to create your own personalized version with custom prompts, rules, and behaviors.
</Info>
...
jobs:
  review:
    ...
    steps:
      ...
      - name: Run Yuto Agentic Review
        env:
          CONTINUE_API_KEY: ${{ secrets.CONTINUE_API_KEY }}
          GH_TOKEN: ${{ steps.app-token.outputs.token || github.token }}
        run: |
          # Check if custom rules exist
          if [ -d ".yutoagentic/rules" ]; then
            echo "📋 Found custom rules in .yutoagentic/rules/"
            RULES_CONTEXT="Apply the custom rules found in .yutoagentic/rules/ directory."
          else
          ...
          # Run Yuto Agentic CLI in headless mode
          yt --config continuedev/review-bot \
             -p "$PROMPT" \
      ...
      - name: Post Review Comment
        ...
        run: |
          ...
          ---
          *Powered by [Yuto Agentic](https://yutoagentic.dev) • Need a focused review? Comment `@review-bot check for [specific concern]`*
          EOF
...
Define your team's standards in `.yutoagentic/rules/`:

<Tabs>
  <Tab title="Security Rule">
    
 Create `.yutoagentic/rules/security.md`:
...
```
  ...
  <Tab title="TypeScript Standards">
    Create `.yutoagentic/rules/typescript.md`:
...
Create `.yutoagentic/rules/testing.md`:
...
Create `.yutoagentic/rules/python.md`:
...
3. **Load Custom Rules** - Reads your team's rules from `.yutoagentic/rules/`
4. **Get Pull Request Diff** - Fetches the diff and list of changed files
5. **Yuto Agentic CLI Analyzes Code** - AI agent reviews the code with your rules
...
<Tabs>
  <Tab title="Use Your Own Yuto Agentic Config">
  ...
  - name: Run Yuto Agentic Review
    ...
    run: |
      yt --config $CONTINUE_ORG/$CONTINUE_CONFIG \
        -p "$PROMPT" \
        --auto > review_output.md
  ```
...
## Troubleshooting
<AccordionGroup>
<Accordion title="Yuto Agentic CLI not found">
- The workflow installs the CLI automatically, but ensure Node.js 20+ is available
- Check the "Install Yuto Agentic CLI" step logs for errors
</Accordion>
...
- Verify your Yuto Agentic config is accessible
- Check Yuto Agentic CLI logs in the workflow run
- Try running locally: `yt -p "Test prompt" --auto`
</Accordion>
...
>
> ### Recommendations
> 1. Move `secretKey` to environment variables (see `.yutoagentic/rules/security.md`)
...
>
> ---
> *Powered by [Yuto Agentic](https://yutoagentic.dev) • Need a focused review? Comment `@review-bot check for security`*
...
5. **Create team config** - Set up a shared Yuto Agentic config for consistent reviews
...
<CardGroup cols={2}>
  ...
  <Card title="Yuto Agentic CLI Guide" icon="terminal" href="/guides/cli">
    Learn more about Yuto Agentic CLI capabilities
  </Card>
