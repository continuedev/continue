...
<Card title="What You'll Build" icon="wand-magic-sparkles">
  A communication workflow assistant that uses Continue CLI with Klavis AI's Strata MCP to manage and automate multi-platform workflow including Slack and Gmail - all through simple natural language prompts.
</Card>
...
- A Klavis [AI account](https://www.klavis.ai/) and API key
- Authenticated connections to the tools you want to use
  ...
  <Tab title="Manual Setup">
    <Steps>
      ...
      <Step title="Add Klavis AI Strata MCP">
        ...
        Notes:
        - Keep `Klavis Strata URL` secret and safe
  ...
  <Tab title="=� Manual CLI Setup">
    <Steps>
      ...
      <Step title="Launch Continue with Klavis MCP">
        ...
        ```
        Now try: "Send a Slack message to #general saying 'Hello from Continue!'"
      </Step>
    </Steps>
  </Tab>
</Tabs>
...
<Card title="Send Channel Message" icon="message">
  Post messages to Slack channels with formatting.
...
**Prompt:**
```
Create a new Slack channel called #project-phoenix
```
</Card>
...
**Prompt:**
```
Find all channels with no messages in the last 90 days.
```
</Card>
...
<Card title="Send Email" icon="envelope">
  Compose and send emails with formatting.
...
**Prompt:**
```
Search my Gmail for emails from the last 30 days containing "invoice" or "payment".
```
</Card>
...
**Prompt:**
```
Send an email to client@company.com with subject "Monthly Report - March 2024"
```
</Card>
...
**Prompt:**
```
Send an announcement about Scheduled Maintainance @ Sat 2-6 AM EST to #engineering channel and also mail to engineering@klavis.ai
```
</Card>
...
**Prompt:**
```
A critical bug has been reported (issue #2847 - payment processing failure).

1. Send urgent Slack message to #engineering-alerts with details
2. Send email to engineering-leads@company.com with:
   - Issue description
   - Impact assessment
   - Immediate action items
3. Create Slack thread for coordination and updates
```
</Card>
...
Leverage Klavis AI Strata's 40+ integrations for comprehensive automation.

<Info>
  Klavis AI Strata supports integrations with Notion, Linear, GitHub, Asana, Salesforce, HubSpot, and more. Combine these with Slack and Gmail for powerful cross-platform workflows.
</Info>
...
Run headless Klavis AI workflows on a schedule or in response to events.
...
- `KLAVIS_API_KEY`: Your Klavis AI API key

### Example Workflow

Create `.github/workflows/klavis-communication.yml`:

```yaml
name: Klavis AI Communication Workflows

on:
  schedule:
    - cron: "0 9 * * 1"  # Monday mornings 9:00 UTC
    - cron: "0 17 * * 5"  # Friday afternoons 17:00 UTC
  workflow_dispatch:
  push:
    branches:
      - main
...
jobs:
  weekly-updates:
    ...
    env:
      ...
      KLAVIS_API_KEY: ${{ secrets.KLAVIS_API_KEY }}
    steps:
      ...
      - name: Send Weekly Planning Message
        run: |
          cn --agent your-klavis-agent \
             -p "Send Slack message to #engineering:
             '=K Good morning team! Week starting [date].
             This week's priorities:
             - Review PRs from last week
             - Sprint planning on Wednesday
             - Deploy v2.3.0 on Friday

             Check Linear for your assigned tasks. Have a great week! =�'" \
             --auto
  ...
  deployment-notification:
    ...
    steps:
      ...
      - name: Send Deployment Notifications
        run: |
          cn --agent your-klavis-agent \
             -p "A new commit was pushed to main branch.

             1. Send Slack message to #deployments:
             '=� New deployment to production in progress
             Commit: ${{ github.sha }}
             Author: ${{ github.actor }}
             Track status: ${{ github.server_url }}/${{ github.repository }}/actions/runs/${{ github.run_id }}'

             2. Email engineering-notifications@company.com with deployment details" \
             --auto
  ...
  friday-digest:
    ...
    steps:
      ...
      - name: Generate Weekly Digest
        run: |
          cn --agent your-klavis-agent \
             -p "Generate end-of-week summary:

             1. Review this week's merged PRs and closed issues
             2. Create summary with key accomplishments
             3. Send to Slack #team-updates with celebration tone
             4. Email weekly-digest@company.com for records

             Include: PR count, issues closed, top contributors, notable features shipped" \
             --auto
```

<Info>
  Customize the agent name `your-klavis-agent` to match your Continue Hub agent configuration.
</Info>
...
<AccordionGroup>
  <Accordion title="Authentication Issues">
    - **Klavis Strata URL not recognized**: Verify the key is correctly set in environment variables or Hub secrets
    - **Slack OAuth failed**: Re-authenticate through Klavis AI dashboard, ensure workspace permissions are granted
    - **Gmail OAuth failed**: Check Google Cloud Console for API access, ensure Gmail API is enabled
  ...
  <Accordion title="Workflow Execution Issues">
    ...
    - **Integration not available**: Verify the integration is active in your Klavis dashboard (klavis.ai/integrations)
  </Accordion>
</AccordionGroup>
...
- ✅ **Manages Gmail**: Read, send, triage emails with intelligent automation
- ✅ **Cross-platform coordination** Unified workflows across Slack, Gmail, and other tools
- ✅ **Scales reliably** on complex multi-app workflows
...
<CardGroup cols={2}>
  <Card title="Scalable Tool Integration" icon="chart-line">
  ...
  <Card title="Smart Navigation" icon="terminal">

     AI drills down **layer by layer** to find the exact tool needed for the task

  </Card>

  <Card title="Precise Execution" icon="camera">
...
<CardGroup cols={2}>
  <Card title="Klavis AI Platform" icon="link" href="https://www.klavis.ai/">
    Official Klavis AI website and documentation
  </Card>
  <Card title="Strata MCP Documentation" icon="book" href="https://www.klavis.ai/strata">
    Learn about Klavis AI's MCP platform architecture
  </Card>
