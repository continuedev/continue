...
We recommend starting with Ollama for its simplicity and reliability. For comprehensive setup instructions, installation troubleshooting, and performance optimization, see our [complete Ollama guide](/guides/ollama-guide).
...
<Note>
  If your machine is truly offline and cannot connect to download models, you
  can download them on a different computer and copy the `~/.ollama` directory
  to your offline machine. See [this Ollama issue
  comment](https://github.com/ollama/ollama/issues/696#issuecomment-1711230525)
...
<Info>
  Setting `contextLength` limits how much code the model can see at once. Lower
  values (2048-4096) improve speed significantly on local machines, while higher
  values (8192+) provide more context but may be slower. Adjust based on your
...
Based on our [model recommendations](/customization/models#recommended-models), here are the best local models for offline development:
...
| --------------- | ------------------------------------------------------------------------ | ------------ | -------------------------------------------------- |
| General coding  | [llama3.1:8b](https://hub.continue.dev/ollama/llama3.1-8b)               | 8GB+         | [Ollama Guide](/guides/ollama-guide)               |
| Light editing   | [gemma2:2b](hhttps://hub.continue.dev/ollama/gemma2-2b)                  | 4GB+         | [Ollama Guide](/guides/ollama-guide)               |
