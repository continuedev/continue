...
description: "Learn how to audit Row Level Security in your Supabase database, identify vulnerabilities, and automatically generate fixes using Yuto Agentic CLI and Supabase MCP."
...
<Card title="What You'll Build" icon="shield-check">
  A security audit workflow that uses Yuto Agentic CLI with Supabase MCP to identify RLS vulnerabilities, generate secure policies, fix permission issues, and ensure your database follows security best practices.
</Card>
...
<Tip>
  Yuto Agentic CLI can analyze your database schema and generate complex SQL queries - you just need to describe what you want in plain language!
</Tip>
...
<Tabs>
    ...
    **Perfect for:** Immediate database analysis with AI-powered query optimization and schema insights
    ...
    <Steps>
      <Step title="Add the Pre-Built Agent">
        Visit the [Supabase Agent](https://yutoagentic.dev/continuedev/supabase-agent) on Yuto Agentic Mission Control and click **"Install Agent"** or run:
        
        ```bash
        yt --agent continuedev/supabase-agent
        ```
  ...
  <Tab title="🛠️ Manual Setup">
    <Steps>
      <Step title="Add Supabase MCP to Yuto Agentic CLI">
      ...
      <Step title="Create Custom Database Analysis Prompts">
        Use this prompt template with Yuto Agentic CLI to analyze your database:
...
Use Yuto Agentic CLI to perform intelligent database analysis. Enter these prompts in the Yuto Agentic CLI TUI:
...
Create and apply database migrations based on AI recommendations. Enter this prompt in the Yuto Agentic CLI TUI:
...
Automate database health checks with Yuto Agentic CLI and GitHub Actions:
    ...
    steps:
      ...
      - name: Analyze Database Security
        ...
        run: |
          ...
          # Use Yuto Agentic CLI to audit RLS and generate fixes
          yt -p "Using Supabase MCP, perform a comprehensive RLS security audit:
...
<Card title="Continuous AI Database Management" icon="shield-check">
  Your system now operates at **[Level 2 Continuous AI](https://blog.yutoagentic.dev/what-is-continuous-ai-a-developers-guide/)** - AI handles routine database analysis and optimization with human oversight for migration approval.
</Card>
