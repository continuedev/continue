...
**Yuto Agentic Implementation**: Using [Chat](/ide-extensions/chat/quick-start) or [Edit](/ide-extensions/edit/quick-start) mode for one-off coding tasks.
...
**Yuto Agentic Implementation**: Using [Yuto Agentic CLI](/guides/cli) with custom rules and integrations into CI/CD pipelines.
...
**Yuto Agentic Implementation**: Fully automated [agents](/ide-extensions/agent/quick-start) with strict permissions and safety guardrails.
...
git diff | yt -p "review this diff and suggest improvements following our team standards"

# Example: Generate missing tests
yt -p "create unit tests for the functions in src/auth.js"

# Example: Update documentation
yt -p "update the README with the new API endpoints from the recent changes"
```
...
Use Yuto Agentic CLI's permission system to gradually expand AI capabilities:

```yaml ~/.yutoagentic/permissions.yaml
permissions:
...
<Card title="Want to dive deeper?" icon="rocket">
      Check out our guides on [Yuto Agentic CLI](/guides/cli) and [Understanding
