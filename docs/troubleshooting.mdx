...
description: "Comprehensive guide to resolving common issues with Shadow Code, including logging, keyboard shortcuts, networking problems, model capabilities, and extension configuration troubleshooting"
---
...
#### Prompt Logs (Shadow Code Console)

To view prompt logs/analytics, you can enable the Shadow Code Console.

1. Open VS Code settings (`cmd/ctrl` + `,`)
2. Search for the setting "Shadow Code: Enable Console" and enable it
3. Reload the window
4. Open the Shadow Code Console by using the command palette (`cmd/ctrl` + `shift` + `P`) and searching for "Shadow Code: Focus on Shadow Code Console View"

![Shadow Code Console](/images/images/continue-console-d387a10c2918c117c6c253a3b5f18c22.png)

### JetBrains

Open `~/.shadow-code/logs/core.log` to view the logs for the Shadow Code plugin. The most recent logs are found at the bottom of the file.
...
We are constantly making fixes and improvements to Shadow Code, but the latest changes remain in a "pre-release" version for roughly a week so that we can test their stability. If you are experiencing issues, you can try the pre-release by going to the Shadow Code extension page in VS Code and selecting "Switch to Pre-Release" as shown below.
...
If your keyboard shortcuts are not resolving, you may have other commands that are taking precedence over the Shadow Code shortcuts. You can see if this is the case, and change your shortcut mappings, in the configuration of your IDE.
...
If Shadow Code can reach your model endpoint but cannot verify its TLS certificate chain, you may see `fetch failed` alongside errors such as:
...
1. Reproduce the error in the Shadow Code logs so you can confirm the exact hostname that failed.
...
5. Verify the fix with curl before retrying in Shadow Code:
...
You can create a GitHub issue here, providing details of your problem, and we'll be able to help you out more quickly.
