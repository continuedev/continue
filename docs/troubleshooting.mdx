...
1. **Check model support**: Not all models support tool/function calling. Models like GPT-3.5 or older Claude versions may not support tools.

2. **Override capabilities**: If Continue's autodetection isn't working correctly, you can manually specify capabilities in your `config.yaml`:
   ```yaml
   models:
     - name: my-model
       provider: openai
...
3. **Verify with provider**: Some proxy services (like OpenRouter) or custom deployments may not preserve tool calling capabilities. Check your provider's documentation.

See the [Model Capabilities guide](/customize/deep-dives/model-capabilities) for detailed troubleshooting steps.
