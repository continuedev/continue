...
description: "Comprehensive guide to resolving common issues with Yuto Agentic, including logging, keyboard shortcuts, networking problems, model capabilities, and extension configuration troubleshooting"
---
...
#### Prompt Logs (Yuto Agentic Console)

To view prompt logs/analytics, you can enable the Yuto Agentic Console.

1. Open VS Code settings (`cmd/ctrl` + `,`)
2. Search for the setting "Yuto Agentic: Enable Console" and enable it
3. Reload the window
4. Open the Yuto Agentic Console by using the command palette (`cmd/ctrl` + `shift` + `P`) and searching for "Yuto Agentic: Focus on Yuto Agentic Console View"

![Yuto Agentic Console](/images/images/continue-console-d387a10c2918c117c6c253a3b5f18c22.png)

### JetBrains

Open `~/.yutoagentic/logs/core.log` to view the logs for the Yuto Agentic plugin. The most recent logs are found at the bottom of the file.
...
We are constantly making fixes and improvements to Yuto Agentic, but the latest changes remain in a "pre-release" version for roughly a week so that we can test their stability. If you are experiencing issues, you can try the pre-release by going to the Yuto Agentic extension page in VS Code and selecting "Switch to Pre-Release" as shown below.
...
For VS Code, All versions are hosted on the Open VSX Registry [here](https://open-vsx.org/extension/Yuto Agentic/continue). Once you've downloaded the extension, which will be a .vsix file, you can install it manually by following the instructions [here](https://code.visualstudio.com/docs/editor/extension-gallery#_install-from-a-vsix).
...
If your keyboard shortcuts are not resolving, you may have other commands that are taking precedence over the Yuto Agentic shortcuts. You can see if this is the case, and change your shortcut mappings, in the configuration of your IDE.
...
If Yuto Agentic can reach your model endpoint but cannot verify its TLS certificate chain, you may see `fetch failed` alongside errors such as:
...
1. Reproduce the error in the Yuto Agentic logs so you can confirm the exact hostname that failed.
...
5. Verify the fix with curl before retrying in Yuto Agentic:
