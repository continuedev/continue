...
<Steps>
  <Step title="Install the documentation agent">
    Add the pre-configured Docs Assistant to your Continue setup. This agent comes pre-configured with all our documentation standards.
...
<Tip>
  You can also remix this config to customize it for your specific needs.
</Tip>
...
<Steps>
      <Step title="Create a new config">
          Create a new `config.yaml` to set up your own configuration. See the [config reference](/reference) for details.
  </Step>
  
  <Step title="Add the Continue Docs MCP">
    Add the Continue Docs MCP to your configuration. This MCP provides context about Continue's documentation structure and standards.
  </Step>
  
  <Step title="Add Mintlify formatting rules">
    Add a Mintlify technical writing rule to your configuration. This rule ensures proper Mintlify component formatting.
  </Step>
...
- **Continue agent questions**: Check the [Continue documentation](https://docs.continue.dev)
