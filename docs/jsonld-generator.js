...
(function () {
  ...
  function generatePageSpecificJSONLD() {
    ...
    const pageDescription = getMetaDescription() || baseOrganizationData.description;
    const canonicalUrl = getCanonicalUrl() || `https://docs.continue.dev${currentPath}`;

    let jsonLDData;

    // Generate different structured data based on the current page
    if (currentPath === "/" || currentPath === "/index" || currentPath.includes("/home")) {
    ...
    } else if (currentPath.includes("/getting-started") || currentPath.includes("/install")) {
    ...
    } else if (currentPath.includes("/customize/") || currentPath.includes("/customization/")) {
      ...
      jsonLDData = {
        ...
        "about": {
          "@type": "Product",
          "name": "Continue Hub",
          "description": "Platform for creating, sharing, and using custom AI code assistants"
        }
      };
    ...
    } else if (currentPath.includes("/troubleshooting") || currentPath.includes("/faqs")) {
  ...
  function extractHowToSteps() {
    ...
    return steps.length > 0 ? steps.slice(0, 10) : [{
      "@type": "HowToStep",
      "position": 1,
    ...
    headings.forEach((heading) => {
      if (heading.textContent && heading.textContent.includes('?')) {
        faqs.push({
          ...
          "acceptedAnswer": {
            "@type": "Answer",
            "text": getNextParagraph(heading)?.textContent?.trim() || "Please refer to the documentation for detailed information."
          }
        });
      }
    });

    return faqs.length > 0 ? faqs.slice(0, 20) : [{
      "@type": "Question", 
  ...
  function getNextParagraph(element) {
    ...
    while (next && next.tagName !== 'P') {
      ...
      if (!next || ['H1', 'H2', 'H3', 'H4', 'H5', 'H6'].includes(next.tagName)) {
        break;
      }
    }
    return next;
  ...
  function addJSONLDToPage(data) {
    ...
    const existingScripts = document.querySelectorAll('script[data-json-ld-generator="continue"]');
