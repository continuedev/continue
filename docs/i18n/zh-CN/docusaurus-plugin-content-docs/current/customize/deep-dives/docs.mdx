...
如果你设置一个 [嵌入提供者](../model-types/embeddings.mdx) ， `@docs` 将会使用你的嵌入提供者。切换嵌入提供者将会触发你的配置中所有文档网站的重新索引。
...
这是对于 Jetbrains 等效的最小示例，需要设置一个 [嵌入提供者](../model-types/embeddings.mdx) 。预索引文档将 _不会_ 访问。
