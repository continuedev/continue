...
Continue 提供了多个方法来提供你自己的上下文给扩展。

## HTTP 上下文提供者

Continue 可以从你创建和服务的定制的 HTTP 服务器获取上下文。添加 `@HTTP` 上下文提供者到你的配置中，像这样：
...
然后，创建一个服务器响应来自 [HttpContextProvider.ts](https://github.com/continuedev/continue/blob/main/core/context/providers/HttpContextProvider.ts) 创建的请求。查看 `hello` 端点 [context_provider_server.py](https://github.com/continuedev/continue/blob/main/core/context/providers/context_provider_server.py) ，一个使用 FastAPI 的例子。

`"options"` 属性可以用来发送更多的参数到你的端点，将会包含在请求体中。

:::info
以下创建定制上下文提供者的方法已经废弃。我们推荐尽可能使用 HTTP 上下文提供者， [MCP Servers](../context-providers.mdx#model-context-protocol) 和 [Prompt files](../deep-dives/prompt-files.md) 。
:::

## 使用 CustomContextProvider

定制上下文提供者可以在 `config.ts` 文件中实现，放在你的 Continue 全局目录 (`~/.continue` for MacOS, `%USERPROFILE%/.continue` for Windows) 。你可以在你的 `config.ts` 实现 `CustomContextProvider` 接口：
