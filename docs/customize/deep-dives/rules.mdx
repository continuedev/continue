---
title: "How to Create and Manage Rules in Shadow Code"
...
1. Create a folder called `.shadow-code/rules` at the top level of your workspace
2. Add a file called `pirates-rule.md` to this folder.
3. Write the following contents to `pirates-rule.md` and save.

```md title=".shadow-code/rules/pirates-rule.md"
---
name: Pirate rule
---
...
For example, you can say "Create a rule for this", and a rule will be created for you in `.shadow-code/rules` based on your conversation.
</Info>
...
globs: docs/**/*.{md,mdx}
alwaysApply: false
description: Standards for writing and maintaining Shadow Code Docs
---

# Shadow Code Docs Standards
...
You can create project-specific rules by adding a `.shadow-code/rules` folder to the root of your project and adding new rule files.
...
```md title=".shadow-code/rules/typescript.md"
---
...
**Check These**:
1. **File location**: Ensure rules are in `.shadow-code/rules/` (not `.shadow-code/rule/`)
2. **File format**: Rules should be `.md` files with proper YAML frontmatter

### How to Customize Chat System Message

Shadow Code includes a simple default system message for [Agent mode](../../ide-extensions/agent/quick-start) and [Chat](../../ide-extensions/chat/quick-start) requests, to help the model provide reliable codeblock formats in its output.
