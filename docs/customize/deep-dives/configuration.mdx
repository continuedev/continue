...
You can customize built-in tool prompts at the workspace level using the `tools` array in `.continuerc.json`. This is especially useful when working with local models that may struggle with Continue's default tool prompts.

<AccordionGroup>
  <Accordion title="Override tool descriptions">
    Customize how tools are described to the LLM:
  ...
  <Accordion title="Disable specific tools">
    Prevent certain tools from being available:
  ...
  <Accordion title="Customize system message prompts">
    Override the system message descriptions for non-native tool calling:
      ...
      "tools": [
        {
          ...
          "systemMessageDescription": {
            ...
            "exampleArgs": [["filepath", "./docs/README.md"]]
          }
        }
      ]
    }
  ...
  <Accordion title="Override display titles and action phrases">
    Customize the UI display and action phrases:
      ...
      "tools": [
        {
          ...
          "wouldLikeTo": "view {{{ filepath }}}",
          "isCurrently": "viewing {{{ filepath }}}",
...
<Note>
  Tool names match the built-in function names like `read_file`, `run_terminal_command`, `view_diff`, etc. Tools are merged by name during configuration loading.
</Note>
