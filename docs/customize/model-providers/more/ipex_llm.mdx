...
description: "Configure Intel Extension for PyTorch (IPEX-LLM) with Yuto Agentic to run language models with very low latency on Intel CPUs and GPUs, leveraging accelerated Ollama backend"
---
...
IPEX-LLM supports accelerated Ollama backend to be hosted on Intel GPU. Refer to [this guide](https://ipex-llm.readthedocs.io/en/latest/doc/LLM/Quickstart/ollama_quickstart.html) from IPEX-LLM official documentation about how to install and run Ollama serve accelerated by IPEX-LLM on Intel GPU. You can then configure Yuto Agentic to use the IPEX-LLM accelerated `"ollama"` provider as follows:
...
<Tip>
  ...
  that model in Yuto Agentic, you could refer to
