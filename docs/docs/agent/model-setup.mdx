...
If the model is _not_ known to support tools, Agent will be disabled. To designate tool support, a model must be configured with the [`tool_use` capability](../reference.md#models) or specifically marked as supporting tools in Continue's [source code](https://github.com/continuedev/continue/blob/main/core/llm/toolSupport.ts#L4).
