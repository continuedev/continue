...
<Tabs groupId="config-example">
  ...
  ```yaml title="config.yaml"
  context:
    - uses: continuedev/postgres-context
      with:
        POSTGRES_HOST: localhost
        POSTGRES_PORT: 5436
        POSTGRES_USER: myuser
        POSTGRES_PASSWORD: catsarecool
        POSTGRES_DATABASE: animals
...
<Tabs groupId="config-example">
  ...
  ```yaml title="config.yaml"
  context:
    - uses: continuedev/gitlab-mr-context
      with:
        GITLAB_TOKEN: "..."
  ```
  </TabItem>
...
<Tabs groupId="config-example">
  ...
  ```yaml title="config.yaml"
  context:
    - uses: continuedev/greptile-context
      with:
        GREPTILE_TOKEN: "..."
        GITHUB_TOKEN: "..."
  ```
