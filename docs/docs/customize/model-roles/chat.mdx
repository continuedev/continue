...
Our current top recommendation is Claude Sonnet 3.7 from [Anthropic](../model-providers/top-level/anthropic.mdx).
...
If you prefer to use a model from [Google](../model-providers/top-level/gemini.mdx), then we recommend Gemini 1.5 Pro.

<Tabs groupId="hub-config-example">
    <TabItem value="hub" label="Hub">
    Add the [Gemini 1.5 Pro block](https://hub.continue.dev/google/gemini-1.5-pro) from the hub
    </TabItem>
  <TabItem value="yaml" label="YAML">
  ```yaml title="config.yaml"
  models:
