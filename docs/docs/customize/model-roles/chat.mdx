...
You can customize the system prompt used for chat interactions by setting the `promptTemplates.baseSystemPrompt` property in your model configuration. Continue uses [Handlebars syntax](https://handlebarsjs.com/guide/) for templating.

Available variables for the chat template:

- `{{{history}}}` - The conversation history
- `{{{system_message}}}` - The default system message (if provided)
...
models:
  - name: My Custom Chat Model
    ...
    promptTemplates:
      baseSystemPrompt: |
        You are an expert coding assistant specialized in JavaScript development.
