...
Some deprecated `config.json` settings are no longer stored in config and have been moved to be editable through the [User Settings Page](/customize/deep-dives/settings). If found in `config.json`, they will be auto-migrated to User Settings and removed from `config.json`.
...
- `ui` - all will override during migration
  ...
  See [User Settings Page](/customize/deep-dives/settings) for more information about each option.
