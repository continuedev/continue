...
Ollama is an open-source tool that allows you to run large language models (LLMs) locally on their own computers. To use Ollama, you can install it [here](https://ollama.ai/download) and download the model you want to run with the `ollama run` command.
...
To configure a remote instance of Ollama, add the `apiBase` property to your model config.
