...
Agent lives within the same interface as [Chat](../chat/how-it-works), so the same [input](../chat/quick-start#type-a-request-and-press-enter) is used to send messages and you can still use the same manual methods of providing context, such as [`@` context providers](../chat/quick-start#reference-context-with-the--symbol) or adding [highlighted code from the editor](../chat/quick-start#highlight-a-code-section-to-include-as-context).
