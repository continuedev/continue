...
2. Open the [User Settings Page](../customize/deep-dives/settings.md) and turn off "Allow Anonymous Telemetry". This will stop Continue from attempting requests to PostHog for [anonymous telemetry](../customize/telemetry.mdx).
3. Also in `config.json`, set the default model to a local model. You can available options [here](../customize/model-providers/top-level/openai.mdx).
