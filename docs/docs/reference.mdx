...
- `promptTemplates`: Can be used to override the default prompt templates for different model roles. Valid values are
  `chat`, [`edit`](./model-roles/edit#prompt-templating), [
  `apply`](./model-roles/apply#prompt-templating) and [
  `autocomplete`](./model-roles/autocomplete#prompt-templating). The `chat` property must be a
...
Destinations to which [development data](./deep-dives/development-data) will be sent.
