...
<Steps>
  ...
  <Step title="Connect Atlassian">
    ...
    Click Setup -> "Connect" next to Atlassian. 
...
Process multiple tickets with natural language commands:

<Card title="Bulk Ticket Processor" icon="list-check">

  **Task Example**: "Update all tickets tagged 'needs-review' with current status from linked PRs"
  **What the Agent Does**:




  - Searches Jira using natural language queries

  **Run in Mission Control**: Run manually or on schedule
</Card>
...
<Steps>
  ...
  <Step title="Create a Unified Development Agent">
    ...
    Build an agent that:
    - Extracts Jira ticket IDs from PRs automatically
    - Updates ticket status based on PR state
    - Translates code changes into business summaries
    - Posts bidirectional links between Jira and GitHub

  </Step>

  <Step title="Set Up Automated Workflow">

    Configure the "Update Jira Tickets" workflow to run on every PR merge
...
Extend the base workflow with these powerful features:
