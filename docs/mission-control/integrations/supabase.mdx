...
<Steps>
  ...
  <Step title="Connect Supabase">
    ...
    Click Setup -> "Connect" next to Supabase. 
...
<Accordion title="🟠 High - Important Security Gap">
...
Combine Supabase with GitHub for complete database security automation:

<Steps>

  <Step title="Connect Both Integrations">

    Enable both Supabase and GitHub integrations in Mission Control

  </Step>

  <Step title="Create a Security Pipeline">

    Build an agent that:
    - Monitors PRs for database changes
    - Audits RLS policies automatically
    - Generates security fixes
    - Commits migrations to the PR branch
    - Posts detailed audit reports

  </Step>

  <Step title="Set Up Automated Auditing">

    Configure the agent to run automatically on all PRs that modify:
    - Migration files
    - Database queries
    - Schema definitions
...
Track your agent's database security performance:

1. **View in Mission Control**: See all agent runs and security fixes
2. **Check PR Comments**: Review detailed audit reports on each PR
3. **Verify Migrations**: Ensure generated SQL follows best practices
4. **Monitor Metrics**: Track security gap trends over time
