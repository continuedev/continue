...
Connect **Supabase** to Mission Control so agents can automatically review **Row Level Security (RLS)** when pull requests touch database-related code. The official workflow focuses on the tables your PR actually references, fixes high-impact gaps, and reports results back in a single PR comment.
