...
description: "Comprehensive guide to the config.yaml format used by Yuto Agentic.dev for building custom coding agents. Learn how to define models, context providers, rules, prompts, and more using YAML configuration."
---

## Introduction

Yuto Agentic Agents are defined using the `config.yaml` specification.
...
Learn how to work with Yuto Agentic's configuration system, including using hub models, rules, and tools, creating local configurations, and organizing your setup.
</Card>
...
- `capabilities`: Array of strings denoting model capabilities, which will overwrite Yuto Agentic's autodetection based on provider and model. See the [Model Capabilities guide](/customize/deep-dives/model-capabilities) for detailed information. Supported capabilities include:
  ...
  Yuto Agentic automatically detects these capabilities for most models, but you can override this when using custom deployments or if autodetection isn't working correctly.
...
schema: v1
rules:
  - uses: sanity/sanity-opinionated # rules file stored on Yuto Agentic Mission Control
...
schema: v1
prompts:
  - uses: supabase/create-functions # prompts file stored on Yuto Agentic Mission Control 
...
docs:
  - name: Yuto Agentic
    startUrl: https://docs.yutoagentic.dev/intro
    favicon: https://docs.yutoagentic.dev/favicon.ico
```
...
The [Model Context Protocol](https://modelcontextprotocol.io/introduction) is a standard proposed by Anthropic to unify prompts, context, and tool use. Yuto Agentic supports any MCP server with the MCP context provider.
