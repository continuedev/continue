...
- [`data`](#data)
- [`disableCommitSignature`](#disablecommitsignature)
...
If set to `true`, disables the automatic inclusion of Continue's attribution signature in commit messages generated by the CLI.

By default, when the Continue CLI generates commit messages, it includes a signature like:
...
Setting this to `true` will prevent this signature from being added.

**Example:**

```yaml title="config.yaml"
disableCommitSignature: true
```
