...
if (Test-Path ".nvmrc") {
    ...
    if ($null -ne $node) {
        try {
            $currentNodeVersion = (& node -v).Trim()
        } catch {
            $currentNodeVersion = ""
        }

        if (-not [string]::IsNullOrWhiteSpace($currentNodeVersion) -and -not [string]::IsNullOrWhiteSpace($requiredNodeVersion)) {
            ...
            if ($requiredVersion -ne $currentVersion) {
                Write-Host "`nWarning: Your Node.js version ($currentNodeVersion) does not match the required version ($requiredNodeVersion)" -ForegroundColor Yellow
        ...
        Write-Host "Skipping Node.js version check because node is not installed." -ForegroundColor Yellow
    }
}
