...
export const ASSISTANTS_FOLDER = `.sasva/${ASSISTANTS}`;
...
export function getDotContinueSubDirs(
  ...
  // Workspace .sasva/<subDirName>
  if (options.includeWorkspace) {
    fullDirs = workspaceDirs.map((dir) =>
      joinPathsToUri(dir, ".sasva", subDirName),
    );
  }
...
/**
 * This method searches in both ~/.sasva and workspace .sasva
 * for all YAML files in the specified subdirctory, for example .sasva/assistants or .sasva/prompts
 */
