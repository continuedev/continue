...
import { getAutocompleteLanguageInfoForFile } from "../../autocomplete/constants/AutocompleteLanguageInfo";
...
export async function deterministicApplyLazyEdit(
  ...
  if (!findInAst(newTree.rootNode, isLazyBlock)) {
    ...
    if (firstSimilarNode?.parent?.equals(oldTree.rootNode)) {
      ...
      const language = getAutocompleteLanguageInfoForFile(filename);
      if (language) {
