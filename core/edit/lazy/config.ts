...
export function validateLazyEditConfig(config: LazyEditConfig): LazyEditConfig {
  ...
  validated.maxProcessingTime = Math.max(
    1000,
    Math.min(30000, validated.maxProcessingTime),
  );
  validated.complexityThreshold = Math.max(
    0.1,
    Math.min(1.0, validated.complexityThreshold),
  );
  validated.confidenceThreshold = Math.max(
    0.1,
    Math.min(1.0, validated.confidenceThreshold),
  );
  validated.maxCacheSize = Math.max(10, Math.min(1000, validated.maxCacheSize));
  validated.cacheTimeout = Math.max(
    60000,
    Math.min(3600000, validated.cacheTimeout),
  ); // 1 min to 1 hour
