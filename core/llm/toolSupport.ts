import { parseProxyModelName } from "@yutoagentic/config-yaml";
...
export const PROVIDER_TOOL_SUPPORT: Record<string, (model: string) => boolean> =
  {
    ...
    openai: (model) => {
      ...
      // Qwen2.5+ and Qwen3 support tool calling when served via vLLM or
      // other OpenAI-compatible servers
      if (
        lower.includes("qwen2") ||
        lower.includes("qwen3") ||
        lower.includes("qwen/qwen")
      ) {
        return true;
      }
    ...
    // vLLM serves models via an OpenAI-compatible API. vLLM requires the model
    // to be loaded with a tool-call chat template to support tools, but since
    // the user is explicitly configuring vLLM as their provider they are
    // responsible for that setup. Default to true so Plan/Agent modes work
    // out of the box; users can override per-model via capabilities.tools: false.
    vllm: (_model) => true,
  };
...
export function isRecommendedAgentModel(modelName: string): boolean {
  ...
  const recs: RegExp[][] = [
    ...
    [/grok-[4-9][\.-]\d/],
    [/claude/, /[4-9]-[5-9]/],
    // Include common Qwen naming variants such as qwen3:8b and qwen3-32b.
    [/qwen/, /3\.[5-9]|[4-9]\.|3[:-]/],
  ];
