...
describe('Rule colocation - glob pattern matching', () => {
  ...
  describe('General rule behavior', () => {
    it('should apply general rules (no globs) to any file', () => {
      const filePaths = ['src/app.ts', 'src/redux/slice.ts', 'src/components/Button.tsx'];
  ...
  describe('Directory-specific rules', () => {
    ...
    it('should apply component rules to component files', () => {
      const filePaths = ['src/components/Button.tsx', 'src/components/Form.jsx'];
  ...
  describe('Colocated rules in nested directories', () => {
    ...
    const nestedRule: RuleWithSource = {
      ...
      globs: '**/*.ts', // Applies to all TypeScript files in this directory and subdirectories
      source: 'rules-block',
  ...
  describe('Multiple file types with exclusions', () => {
    ...
    it('should not apply to excluded files', () => {
      const filePaths = ['src/utils/helper.test.ts', 'src/components/Button.spec.ts'];
      expect(shouldApplyRule(mixedRule, filePaths)).toBe(false);
    });
