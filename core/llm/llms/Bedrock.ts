...
class Bedrock extends BaseLLM {
  ...
  private async _fetchWithAwsAuthSigV4(apiMethod: BedrockCommon.Method, body: string): Promise<Response> {
    ...
    const opts = {
      headers: {
        "accept": apiMethod === BedrockCommon.Method.Chat ? "application/json" : "application/vnd.amazon.eventstream",
  ...
  protected async *_streamChat(
    ...
    const response = await this._fetchWithAwsAuthSigV4(BedrockCommon.Method.Chat, JSON.stringify({
      ...this._convertArgs(options),
      messages: this._convertMessages(messages),
      anthropic_version: "bedrock-2023-05-31", // Fixed, required parameter for Chat API.
      })
    );
    yield { role: "assistant", content: (await response.json()).content[0].text }
  }
}
