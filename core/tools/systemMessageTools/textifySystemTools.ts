...
export function convertToolCallStatesToSystemCallsAndOutput(
  ...
  const assistantParts = normalizeToMessageParts(originalAssistantMessage);
  ...
  if (userParts.length === 0) {
    userParts.push({
      type: "text",
      text: "No tool output for tool calls",
    });
  }

  const nonEmptyAssistantParts = assistantParts.filter(
    (p) => p.type !== "text" || !!p.text,
  );
  const nonEmptyUserParts = userParts.filter(
    (p) => p.type !== "text" || !!p.text,
  );
