...
const handleGlobalContextUpdate = async (num: number) => {
  globalContext.update("maxNumberOfConcurrentIndexing", num);
};
...
export class EmbeddingManager {
  ...
  constructor() {
    const presistConcurrentNumber = globalContext.get(
      "maxNumberOfConcurrentIndexing",
    );
    if (presistConcurrentNumber) {
      this.maxNumberOfConcurrentProcesses = presistConcurrentNumber;
    } else {
      void handleGlobalContextUpdate(NUM_CONCURRENT_EMBEDDINGS);
    }
  }
  ...
  private async embed(
    ...
    while (!done) {
      ...
      if (result.done) {
        ...
        const toWait = 100 * this.currentIndexing.size + 50;
    ...
    for (const page of pages) {
      ...
      const toWait = 20 * this.currentIndexing.size + 10;
      await new Promise((resolve) => setTimeout(resolve, toWait));
    }
  ...
  /**
   * Retrieves the current maximum number of concurrent indexing processes.
   * @returns The maximum number of concurrent processes.
   */
  public getMaxNumberOfConcurrentIndexing() {
    return this.maxNumberOfConcurrentProcesses;
  }

  /**
   * Updates the maximum number of concurrent indexing processes.
   * @param num - The new maximum number of concurrent processes.
   */
  public updateNumberOfConcurrentIndexing(num: number) {
    void handleGlobalContextUpdate(num);
    this.maxNumberOfConcurrentProcesses = num;
  }
}
