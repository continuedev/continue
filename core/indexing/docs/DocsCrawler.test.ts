...
describe("DocsCrawler", () => {
  ...
  //   expectedCrawlerUsed: DocsCrawlerType,
  // ) {
  ...
  //         if (result.done) {
  //           done = true;
  //           expect(result.value).toEqual(expectedCrawlerUsed);
  //         } else {
  ...
  // describe("GitHub Crawler", async () => {
  //   docsCrawler = new DocsCrawler(mockIde, config, 5, 5, false);

  //   const repoUrl =
  //     "https://github.com/Patrick-Erichsen/test-github-repo-for-crawling";

  //   const crawlResults = await crawlTest(repoUrl, 1, "github");
  ...
  //   crawlTest("https://amplified.dev/", 1, "default");
  // });

  describe("Cheerio Crawler", () => {
    docsCrawler = new DocsCrawler(mockIde, config, 5, 5, true);

    // crawlTest("https://amplified.dev/", 1, "cheerio");
  });
});
