...
function createEmptyTaskState(): AgentTaskState {
  return {
    nextId: 1,
    tasks: [],
  };
...
function requireSessionId(sessionId: string): string {
  ...
  if (!normalized) {
    throw new Error("A non-empty sessionId is required for task state.");
  }
  return normalized;
}

async function loadTaskState(sessionId: string): Promise<AgentTaskState> {
...
export async function listAgentTasks(sessionId: string): Promise<AgentTask[]> {
...
export async function createAgentTask(
  ...
  const state = await loadTaskState(normalizedSessionId);
  const now = Date.now();
  ...
  await saveTaskState(normalizedSessionId, state);
  return task;
}
...
export async function stopAgentTask(
  ...
  return updateAgentTask(sessionId, taskId, {
    status: "cancelled",
