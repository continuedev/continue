...
describe("uri utils", () => {
  ...
  describe("relativePathOrUriToUri", () => {
    ...
    it("should return original URI if scheme exists", () => {
      const uri = "file:///path/to/file.txt";
      expect(relativePathOrUriToUri(uri, "file:///base")).toBe(uri);
    });

    it("should convert relative path to URI", () => {
      expect(relativePathOrUriToUri("path/to/file.txt", "file:///base")).toBe(
        "file:///base/path/to/file.txt",
      );
      expect(consoleSpy).toHaveBeenCalledWith("Received path with no scheme");
    });
  });
