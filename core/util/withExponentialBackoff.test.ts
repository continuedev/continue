...
const MAX_RETRIES = 4;
const INITIAL_DELAY_SECONDS = 0.01;
...
describe("withExponentialBackoff", () => {
  ...
  it("should stop retrying after maxRetries and throw error", async () => {
    const apiCall = jest.fn().mockRejectedValue({
      response: { status: 429, headers: new Map([[RETRY_AFTER_HEADER, "1"]]) },
    });

    await expect(
      withExponentialBackoff(apiCall, MAX_RETRIES, INITIAL_DELAY_SECONDS),
