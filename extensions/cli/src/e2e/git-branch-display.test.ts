  ...
  describe("git branch display in status bar", () => {
    it("should show branch name when in a git repository", async () => {
      ...
      try {
        ...
        await fs.writeFile(path.join(context.testDir, "test.txt"), "test content");
    ...
    it("should handle detached HEAD state", async () => {
      ...
      try {
        ...
        await fs.writeFile(path.join(context.testDir, "test2.txt"), "test content 2");
        execSync("git add .", { stdio: "ignore" });
        ...
        console.warn("Skipping detached HEAD test - git operations failed:", error);
        return;
      }
    });
    ...
    it("should handle branch names with special characters", async () => {
      ...
      try {
        ...
        // Skip test if git operations fail
        console.warn("Skipping special characters test - git operations failed:", error);
        return;
      }
    });
  });
  ...
  describe("getRepoUrlText function with branch", () => {
    it("should format repo URL with branch correctly", async () => {
      ...
      try {
        ...
        // Add a fake remote
        execSync("git remote add origin https://github.com/testuser/testrepo.git", { stdio: "ignore" });
        ...
        const { getResponsiveRepoText } = await import("../ui/hooks/useTUIChatHooks.js");
        ...
        // Skip test if git operations fail
        console.warn("Skipping repo URL formatting test - git operations failed:", error);
        return;
      }
    });
