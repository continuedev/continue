...
This is a CLI tool for Shadow Code Dev that provides an interactive AI-assisted development experience. The architecture consists of:
...
3. **Shadow Code SDK Integration** (`src/continueSDK.ts`): Initializes the Shadow Code SDK client with:
...
7. **Hooks System** (`src/hooks/`): Event interception system for extending CLI behavior
   ...
   - **Config locations** (lowest to highest precedence):
     - `~/.claude/settings.json`, `~/.shadow-code/settings.json` (user-global)
     - `.claude/settings.json`, `.shadow-code/settings.json` (project)
     - `.claude/settings.local.json`, `.shadow-code/settings.local.json` (project-local)
