# OTLP Metrics Specification for Shadow Code CLI

This document specifies the OpenTelemetry Protocol (OTLP) metrics that should be emitted by the Shadow Code CLI to provide comprehensive observability and usage monitoring. The metrics are designed to be compatible with Claude Code dashboards for easy migration.

## Overview

The Shadow Code CLI should emit metrics that provide insights into:
...
| `app.version`       | Current Shadow Code CLI version                               | `OTEL_METRICS_INCLUDE_VERSION` (default: false)     |
...
These metrics are unique to Shadow Code CLI and provide additional insights without conflicting with compatibility:
