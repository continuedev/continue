...
2. **File-Based Auth**: `~/.shadow-code/auth.json` file
...
1. **CLI `--config` Flag** (highest priority)
   ...
   - Assistant slug (`owner/package`): Fetches from Shadow Code platform
...
3. **Default Resolution** (if no flag and no saved URI)
   ...
   - **config.yaml**: The saved config file at `~/.shadow-code/config.yaml`
