  ...
  implements vscode.WebviewViewProvider
{
  ...
  resolveWebviewView(
    ...
    this._webview.onDidReceiveMessage((message: FromLogView) => {
  ...
  constructor(
    ...
    this._saveLog = config.get<boolean>("enableModelLogging");

    vscode.workspace.onDidChangeConfiguration((e) => {
      if (e.affectsConfiguration(`${EXTENSION_NAME}.enableModelLogging`)) {
        const config = vscode.workspace.getConfiguration(EXTENSION_NAME);
        this._saveLog = config.get<boolean>("enableModelLogging");
        if (!this._saveLog) {
          this.clearLog();
