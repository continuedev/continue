...
export class VsCodeExtension {
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    this.configHandler.loadConfig().then(async (config) => {
      ...
      if (!modelTitle) {
        modelTitle = await this.sidebar.webviewProtocol.request(
      ...
      review.onReviewUpdate((result) =>
        this.sidebar.webviewProtocol.request("review/update", [result]),
      );
      this.sidebar.webviewProtocol.on("review/getResults", () => {
        return review.currentResults;
      });
      this.sidebar.webviewProtocol.on("review/redoAll", () => {
        review.redoAll();
      });
    });
