...
interface ToLogViewInit {
  type: "init";
  uuid: string;
  items: LLMInteractionItem[];
}

interface ToLogViewItem {
  type: "item";
  uuid: string;
  item: LLMInteractionItem;
}

interface ToLogViewClear {
  type: "clear";
  uuid: string;
}

type ToLogView = ToLogViewInit | ToLogViewItem | ToLogViewClear;
...
export default function useLLMLog() {
  const [llmLog, dispatchLlmLog] = useReducer(
    (llmLog: LLMLog, message: ToLogView) => {
      switch (message.type) {
        case "init":
  ...
  useEffect(function () {
    ...
    const onMessage = (event: MessageEvent<ToLogView>) => {
