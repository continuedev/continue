...
*/
export async function preprocessToolCallArgs(
...
export const preprocessToolCalls = createAsyncThunk<
  void,
  ToolCallState[],
  ThunkApiType
>(
  "preprocess-tool-calls",
  async (generatedToolCalls, { dispatch, extra, getState }) => {
    // Tool call pre-processing
    await Promise.all(
      generatedToolCalls.map(async (tcState) => {
        try {
          const changedArgs = await preprocessToolCallArgs(
            extra.ideMessenger,
