...
window.addEventListener('error', (event) => {
  if (event.error?.message?.includes('getClientRects') || 
...
window.addEventListener('unhandledrejection', (event) => {
  if (event.reason?.message?.includes('getClientRects') || 
...
Object.defineProperty(Element.prototype, 'getClientRects', {
  value: vi.fn(() => ({
    ...
    item: () => ({ top: 0, bottom: 20, left: 0, right: 100, width: 100, height: 20 })
  }))
});

Object.defineProperty(Element.prototype, 'getBoundingClientRect', {
  value: vi.fn(() => ({ top: 0, bottom: 20, left: 0, right: 100, width: 100, height: 20 }))
});
