...
export function shouldSkipContextProviders(
    ...
    const matchingIntegrationKey = Object.keys(SPECIFIC_CONTEXT_PROVIDERS_INTEGRATIONS).find(key =>
        defaultModelTitle?.toLowerCase().includes(key)
    );

    if (matchingIntegrationKey) {
        const specificProviders = SPECIFIC_CONTEXT_PROVIDERS_INTEGRATIONS[matchingIntegrationKey];
        // Return to skip if the requested title of provider is not in the specificProviders
        return !specificProviders.some(provider => provider === description.title);
    }
...
export function getContextProviders() {
    ...
    if (pathname in SPECIFIC_CONTEXT_PROVIDERS_PATHNAME) {
      ...
      return availableContextProviders.filter(
        (provider) => specificProviders.includes(provider.title)
      );
    }
