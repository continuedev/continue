...
3. Also in `config.json`, set the default model to a local model. You can available options [here](/customize/model-providers/openai).
4. Restart VS Code to ensure that the changes to `config.json` or `config.yaml` take effect.

[Edit this page](https://github.com/continuedev/continue/tree/main/docs/docs/guides/running-continue-without-internet.mdx)
