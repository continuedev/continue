...
To use it, you need to add a documentation site to your config file. See the [docs](/customize/context/documentation) page for more information.
...
Read more about indexing and retrieval [here](/customize/context/codebase).
...
```json
{  "contextProviders": [    {      "name": "issue",      "params": {        "repos": [          {            "owner": "continuedev",            "repo": "continue"          }        ],        "githubToken": "ghp_xxx"      }    }  ]}
```
...
```json
{  "contextProviders": [    {      "name": "google",      "params": {        "serperApiKey": "<YOUR_SERPER.DEV_API_KEY>"      }    }  ]}
```
...
```json
{  "contextProviders": [    {      "name": "gitlab-mr",      "params": {        "token": "...",        "domain": "gitlab.example.com"       }    }  ]}
```
...
```json
{  "contextProviders": [    {      "name": "jira",       "params": {        "domain": "company.atlassian.net",        "token": "ATATT..."      }    }  ]}
```
...
```json
{  "contextProviders": [    {      "name": "discord",      "params": {        "discordKey": "bot token",        "guildId": "1234567890",        "channels": [          {            "id": "123456",            "name": "example-channel"          },          {            "id": "678901",            "name": "example-channel-2"           }        ]      }    }  ]}
```
...
```json
{  "contextProviders": [    {      "name": "http",       "params": {        "url": "https://api.example.com/v1/users"      }    }  ]}
```
...
```json
{  "contextProviders": [    {      "name": "commit",       "params": {        "Depth": 50,        "LastXCommitsDepth": 10      }    }  ]}
```
...
```json
{  "contextProviders": [    {      "name": "greptile",      "params": {        "GreptileToken": "...",        "GithubToken": "..."      }    }  ]}
```
...
[Edit this page](https://github.com/continuedev/continue/tree/main/docs/docs/customize/custom-providers.mdx)
